...
\title{Concentricity of Zero-Spheres for a class of sections of the commutative ring $\mathcal R$\\ of slice preserving functions
...
The tool, designed as a concentricity detector, is built from the section itself, out of
groupoids, orbit--stabilizer, and categorical homotopy theory; what it detects is connectedness. Conditions \textup{(C1)--(C3)} determine one
...
No one can picture an eight-dimensional graph in their head, but tools from categorical homotopy theory, like the total-object Grothendieck construction and standard colimit arguments, let us probe this structure and reason about it. The categorical construction of Part 3 and the zeta-specific equivalence are formalized in Lean, with the classical inputs of Part 1 cited from the literature; the reader can check each formalized step, and see exactly where the connected component ends and the Riemann Hypothesis begins.
...
from the finite plane to the Riemann sphere
\[
  \zeta:\CC\longrightarrow\Cstar,\qquad \Cstar=\CC\cup\{\infty\}=S^2,\qquad \zeta(1)=\infty .
\]
(The domain point at infinity is not included: $\zeta$ has no limit there, Definition~\ref{def:zeta-Cstar}.)
...
Regularity on $\Ostar$ is understood as for $\zetaC$ in Definition~\ref{def:zeta-Cstar}: a
section is slice regular on $\Ostar\setminus\{N\}$ away from its poles, and its value at the
domain point $N$ is assigned data, a real point or $\infty$. No regularity at $N$ is asserted;
for $\zetaO$ none holds.
...
\]
so its restriction to a slice is a self-map of that slice sphere,
\[
  A_v:=\varphi_v^{-1}\circ A\circ\varphi_v:\ \Cstar\longrightarrow\Cstar,
\]
holomorphic on the finite slice plane away from its poles, with the value at the domain point
$\infty$ assigned as in Definition~\ref{def:R}, through the identification
$\varphi_v:\Cstar\xrightarrow{\sim}\CC_v^{*}$ of Definition~\ref{def:slices}. The
\emph{$I$-independent lift} is the fact that this stem is the \emph{same} for every $v$: there is
one $A_\circ:\Cstar\to\Cstar$, holomorphic on $\CC$ away from its poles, with
\[
...
(Definition~\ref{def:base}) is a question about infinitely many states at once.
Theorem~\ref{thm:residue-total-singleton} answers it affirmatively for the C-residue total. That is
a statement about components; whether the infinitely many real reads align is the separate
question of Proposition~\ref{prop:read-equivalence}, and for a general A-section the answer is no
...
\section{The concentricity question}\label{sec:detector}

We now have the worlds and the base, so this section defines the class of functions precisely,
proves what the categorical construction establishes about it, and records what it does not. Two
facts about slice-preserving functions come first: the Weierstrass
...
slice-regular functions on $\OO^{*}=S^8$ \textup{(Definition~\ref{def:R}, regularity understood
on $\OO^{*}\setminus\{N\}$ with the value at $N$ assigned, as there)} with the following
four properties.
...
The Lean structure \texttt{ASection} records, under C3, two further convergence-side clauses that
the printed conditions leave implicit: the real parts of the $q_n$ are bounded below, and
$\sum_n (1+|q_n|^{2})^{-1}<\infty$. Every statement in this paper that quantifies over
A-sections and carries a Lean tag is proved for the structure including these clauses; $\zetaO$
satisfies both (the open strip, and the classical zero density), as does the A-section of
Proposition~\ref{prop:countermodel}.
...
The Lean source realizes the same inverse image on its \emph{semantic input groupoid}
$\mathcal I_A$ \textup{(}\texttt{CResidueInputWorld}\textup{)}: its objects are the M\"obius
input states whose image under the distinguished disk action lies in the residue locus, and its
arrows are the native M\"obius input transports. The C-residue diagram on that base,
$\mathcal R_A^{\mathrm{in}}:\mathcal I_A\to\mathbf{Grpd}$
\textup{(}\texttt{AsectionCResidueInputDiagram}\textup{)}, has as fibres the witnessed states over
each input, and its Grothendieck total $\int_{\mathcal I_A}\mathcal R_A^{\mathrm{in}}$
\textup{(}\texttt{residueTotalCategory}\textup{)} is the object about which transitivity,
connectedness, and the singleton component are proved. Throughout Part~3, the notation
$\int_{\mathcal B}\mathcal R_A$ for the C-residue total refers to this total; the projective-frame
presentation over $\mathcal B$ is retained in the source only for the componentwise inclusion
certificates of $\iota_A$.
\end{definition}
...
Whether they carry one common real value is a separate question; the singleton colimit of
the next step identifies their component classes, not their coordinates
(Theorem~\ref{thm:residue-total-singleton}, Proposition~\ref{prop:read-equivalence}). The comparison above rests on the relative stabilizer cancellation
...
projective great-circle base $\mathcal B$ to the A-generated normalized value groupoids. As
fixed in Definition~\ref{def:residue-subdiagram}, $\int_{\mathcal B}\mathcal R_A$ denotes the
total $\int_{\mathcal I_A}\mathcal R_A^{\mathrm{in}}$ of the semantic C-residue input diagram
\textup{(}\texttt{residueTotalCategory}\textup{)}; the statements below are about that total.
\begin{enumerate}[leftmargin=2.4em]
...
the established conclusion is connectedness. Equality of real coordinates does not follow from
it, and the colimit readout below does not supply it.
...
the zero-sphere through $i$. Then $A$ satisfies \textup{(C1)--(C4)}: the pole is at $1$, and the
assigned value at $N$ is $1$, as for $\zetaO$; the Euler family of $\zetaO$, indexed by the primes,
is augmented by a second infinite family, indexed by $\mathbb{N}$, of geometric scalings
$\tfrac12\,2^{-n}\,\ell$ of one slice-preserving function $\ell$ that is a logarithm of $q^{2}+1$
on the half-space $\re q>1$, each zero-free and slice regular there and summing there to $\ell$; the real Weierstrass factor and the entire factor are those of
...
in \texttt{nontrivialZero\_re\_mem\_Ioo}). Classically, $i$ is not a zero of $\zeta$, since it lies
outside the open strip; the Lean argument does not need this, because nonconcentricity already
follows from the real parts.
\end{proof}
...
Hypothesis the common centre is $\tfrac12$. Conversely the functional equation of $\xi$ pins any
common centre to $\tfrac12$, which is why concentricity forces the Riemann Hypothesis
...
\end{itemize}
After the \textup{(C1)--(C4)} verification, the downstream identification of the common centre
as $\tfrac12$, under the Riemann Hypothesis, uses the
